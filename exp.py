...
if __name__ == '__main__':
	...
	pth_svm_raw_dataset_for_n_back_diff_exp_agg(dataset_name='HCP2021WM', _test_size=.33, _path=RESULT_DIR+'/pth_SVM_agg1.pth',
						_remove_mean=False, _preproc=lambda x:x, _epochs=1000,  _lr=1e-4, lr_decay=.1, _weight_decay=1., _seed=_seed, _cuda=True) #1e-10 SGD
	pth_svm_raw_dataset_for_n_back_diff_exp_agg(dataset_name='HCP2021WM', _test_size=.33, _path=RESULT_DIR+'/pth_SVM_agg2.pth',
						_remove_mean=False, _preproc=lambda x:x, _epochs=1000,  _lr=1e-4, lr_decay=.1, _weight_decay=1., _margin=5., _seed=_seed, _cuda=True) #1e-10 SGD
	pth_svm_raw_dataset_for_n_back_diff_exp_agg(dataset_name='HCP2021WM', _test_size=.33, _path=RESULT_DIR+'/pth_SVM_agg3.pth',
						_remove_mean=True, _preproc=lambda x: (x-x.mean(axis=0, keepdims=True))/x.std(axis=0, keepdims=True), _epochs=1000,  _lr=1e-1, lr_decay=.1, _weight_decay=1., _seed=_seed, _cuda=True) #1e-10 SGD
